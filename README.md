...
|                             名称                              |  バージョン  |              備考               |
| :------------------------------------------------------------ | :----------- | :------------------------------ |
| [Espressif-IDE](https://dl.espressif.com/dl/esp-idf/)         | 2021-09(x64) | Eclipse + ESP-IDFの環境         |
| [Pleiades](https://mergedoc.osdn.jp/index.html#pleiades.html) | -            | Eclipse日本語化プラグイン(任意) |

1. Espressif-IDEのインストーラーをダウンロードする\
   ![alt](image/1-install-download.png)\
   赤枠はオンラインインストーラー\
   紫枠はオフラインインストーラー\
...
1. ダウンロードしたファイルを実行する
1. "English"を選択する\
   ![alt](image/2-install-laugage-select.png)
1. ライセンスに同意する\
   ![alt](image/3-install-license-agreement.png)
1. システムチェックが完了するまで待つ\
   ![alt](image/4-install-system-check.png)\
   途中、250文字以上のパスを使えるようにレジストリを書き換える箇所があるので、出てきた場合は同意する
1. ESP-IDFの選択を行う\
   ![alt](image/5-install-select-esp-idf.png)\
...
1. インストールするESP-IDFのバージョンを選択する\
   ![alt](image/6-install-select-esp-idf-version.png)\
...
1. Espressif-IDEを含むESP-IDF toolのイントールパスを設定する\
   ![alt](image/7-install-select-esp-idf-tool-install-path.png)
1. インストールするコンポーネントを選択する\
   ![alt](image/8-install-select-components.png)\
   画像のように設定する\
   "Chip Targets"は自身が使うものを選択する
1. インストール内容の確認\
   ![alt](image/9-isntall-ready-install.png)
1. インストールを開始すると以下のようになるので完了まで待つ\
   ![alt](image/10-install-driver-install.png)\
   途中、以下のようなのが出た場合はインストールを行う\
   ![alt](image/11-install-driver-install-agree.png)
1. 完了すると以下のような表示になるので、そのまま"Finish"を押下する\
   ![alt](image/12-install-finish.png)
1. PowerShellが起動し、以下のようなプロンプトが流れる\
   ![alt](image/13-install-powershell-env.png)\
   以下のようになったら完了となり、PowerShellを終了してよい\
   ![alt](image/14-install-powershell-env-finish.png)\
   コマンドプロンプトでも同様
1. デスクトップに以下のアイコンが作成される\
   ![alt](image/15-install-finish-create-icon.png)

## 2 Espressif-IDEの初期設定
1. "Espressif-IDE"のショートカットから"Espressif-IDE"を起動する
1. プロジェクトの保存先を選択する\
   ![alt](image/16-initialize-workspace.png)\
   "Use this as the default and do not ask again"にチェックをいれると、以降起動時に聞かれなくなる
1. "Launch"で実行すると以下のように"Espressif-IDE"が起動する
   ![alt](image/17-initialize-welcome.png)
1. 左下に"Installing tools...:(xx%)"と出ている間は待つ\
   "Welcome"タブを閉じると以下のようなものが表示される\
   ![alt](image/18-initialize-intall-tools.png)
1. 完了すると以下のようになる\
   ![alt](image/19-initialize-install-tool-done.png)
1. "Window"→"Preferences"をクリックする\
   ![alt](image/20-initialize-show-preferences.png)
1. "Preferences"ウィンドウが開いたら"Install/Update"→"Automatic Updates"をクリックする\
   ...
   ![alt](image/21-initialize-auto-update-disable.png)
...
1. PCに接続するとデバイスマネージャーからは以下のように表示される\
   "FT232HL"の場合\
   ![alt](image/22-debugger-setting-1-ft232hl.png)\
   "FT2232D"の場合("FT2232H"も？)\
   ![alt](image/22-debugger-setting-1-ft2232d.png)
1. Zadigを起動する
1. "Option"→"List All Device"をクリックしてチェックを入れる\
   ![alt](image/23-debugger-setting-2.png)
1. リストから接続したデバイスを選択する\
   "FT232HL"の場合は以下のようになる\
   ![alt](image/24-debugger-setting-3-ft232hl.png)\
   ...
   ![alt](image/24-debugger-setting-3-ft2232d.png)
1. 以下のように赤枠で囲ったところを"WinUSB"にして"Replace Driver"をクリックする\
   "FT232HL"の場合\
   ![alt](image/25-debugger-setting-4-ft232hl.png)\
   "FT2232D"の場合("FT2232H"も？)\
   ![alt](image/25-debugger-setting-4-ft2232d.png)
1. 完了すると以下の表示が出るので"Close"をクリックする\
   ![alt](image/26-debugger-setting-5.png)
1. Zadigを終了する
1. デバイスマネージャーからは以下のように表示される\
   "FT232HL"の場合\
   ![alt](image/27-debugger-setting-5-ft232hl.png)\
   "FT2232D"の場合("FT2232H"も？)\
   ![alt](image/27-debugger-setting-5-ft2232d.png)
...
| IO23  |  -[^2]  |   BDBUS1    | UART(TX) |
| IO22  |  -[^2]  |   BDBUS0    | UART(RX) |
|  GND  |   GND   |     GND     |          |
...
1. "ユニバーサル シリアル バス デバイス"のリストを右クリックし、アンインストールをする\
   "FT232HL"の場合\
   ![alt](image/28-debugger-uninstall-ft232hl.png)\
   "FT2232D"の場合("FT2232H"も？)\
   ![alt](image/28-debugger-uninstall-ft2232d.png)
...
1. "File"→"New"→"Espressif IDF Project"をクリックする\
   ![alt](image/29-build-create-project.png)
1. "New Project"ウィンドウが出るので以下のようにする(今回はサンプルプロジェクトを実行する)
   ![alt](image/30-build-new-project.png)\
...
1. 以下のように"Terminal"タブが表示される\
   ![alt](image/41-debug-view-terminal-tab.png)
1. 赤枠で囲った箇所をクリックするとプログラムの実行をする\
   ![alt](image/42-debug-run.png)
1. 以下のように、"Terminal"タブから文字列が出力される
   ![alt](image/43-debug-output-terminal.png)
